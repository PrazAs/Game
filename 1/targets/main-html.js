...
urgame_Game.prototype = $extend(flambe_Component.prototype,{
	...
	,init: function() {
		...
		var row = 3;
		var col = 5;
		this.levelSelectorEntity = new flambe_Entity().addChild(new flambe_Entity().add(new flambe_display_ImageSprite(urgame_Main.assetPack.getTexture("LevelSelectBG"))));
		var _g1 = 0;
		var _g = urgame_Game.maxLevel;
		while(_g1 < _g) {
			var i = _g1++;
			var levelItem = new urgame_LevelItem();
			levelItem.updateLevel("" + (i + 1));
			this.levelSelectorEntity.addChild(levelItem.getButton());
			levelItem.setPosition(i % col * (flambe_System.get_stage().get_width() / col) + flambe_System.get_stage().get_width() / col * 0.5 - flambe_display_Sprite.getBounds(levelItem.getButton()).width * 0.5,Math.floor(i / col) * (flambe_System.get_stage().get_height() / row) + flambe_System.get_stage().get_height() / row * 0.5 - flambe_display_Sprite.getBounds(levelItem.getButton()).height * 0.5);
		}
		...
		urgame_Signals.showLevel.connect($bind(this,this.drawLevel)).once();
		urgame_Signals.gameOver.connect($bind(this,this.endGame)).once();
	}
	,showLevels: function() {
		this.owner.addChild(this.levelSelectorEntity);
	}
	,startGameLoop: function(value) {
		this.currentLevel = Std.parseInt(value);
		this.hideLevels();
		this.createLives();
		this.showScore();
		this.showQuestion();
	...
	,hideLevels: function() {
		this.owner.removeChild(this.levelSelectorEntity);
	}
		...
		if(urgame_Game.poppedBalloon != null) {
			if(urgame_Game.poppedBalloon.answerString == this.currentQuestion.correctAnswer) {
				...
				if((function($this) {
					...
					var $r;
					var a1 = $this.currentLevel;
					var b = urgame_Game.maxLevel;
					$r = (function($this) {
						var $r;
						var aNeg = b < 0;
						var bNeg = a1 < 0;
						$r = aNeg != bNeg?aNeg:b > a1;
						return $r;
					}($this));
					return $r;
				}(this))) {
...
var urgame_LevelItem = function() {
	if(urgame_LevelItem.levelDown == null) urgame_LevelItem.levelDown = new flambe_display_ImageSprite(urgame_Main.assetPack.getTexture("LevelButtonDown"));
	if(urgame_LevelItem.levelText == null) urgame_LevelItem.levelText = new flambe_display_TextSprite(urgame_Main.font);
	this.buttonEntity = new flambe_Entity();
	this.levelUp = new flambe_display_ImageSprite(urgame_Main.assetPack.getTexture("LevelButtonUp"));
	this.buttonEntity.add(this.levelUp);
	this.levelTextEntity = new flambe_Entity();
	urgame_LevelItem.levelText = new flambe_display_TextSprite(urgame_Main.font);
	urgame_LevelItem.levelText.set_align(flambe_display_TextAlign.Center);
	this.buttonEntity.addChild(this.levelTextEntity.add(urgame_LevelItem.levelText));
	this.downEntity = new flambe_Entity().add(urgame_LevelItem.levelDown);
	((function($this) {
		var $r;
		var component = $this.buttonEntity.getComponent("Sprite_4");
		$r = component;
		return $r;
	}(this))).get_pointerDown().connect($bind(this,this.handleTouchDown)).once();
	((function($this) {
		var $r;
		var component1 = $this.buttonEntity.getComponent("Sprite_4");
		$r = component1;
		return $r;
	}(this))).get_pointerUp().connect($bind(this,this.handleTouchUp)).once();
};
$hxClasses["urgame.LevelItem"] = urgame_LevelItem;
urgame_LevelItem.__name__ = true;
urgame_LevelItem.prototype = {
	setPosition: function(xVal,yVal) {
		this.levelUp.x.set__(xVal);
		this.levelUp.y.set__(yVal);
	}
	,updateLevel: function(level) {
		urgame_LevelItem.levelText.set_text(this.level = level);
		urgame_LevelItem.levelText.set_align(flambe_display_TextAlign.Center);
		urgame_LevelItem.levelText.x.set__(-flambe_display_Sprite.getBounds(this.levelTextEntity).width * 0.25 + this.levelUp.x.get__() + this.levelUp.texture.get_width() * 0.5);
		urgame_LevelItem.levelText.y.set__(-flambe_display_Sprite.getBounds(this.levelTextEntity).height * 0.5 + this.levelUp.y.get__() + this.levelUp.texture.get_height() * 0.5);
	}
	,handleTouchDown: function(events) {
		this.buttonEntity.addChild(this.downEntity);
		urgame_Signals.startGame.emit(this.level);
	}
	,handleTouchUp: function(events) {
		this.buttonEntity.removeChild(this.downEntity);
		urgame_LevelItem.levelText.set_text("");
	}
	,getButton: function() {
		return this.buttonEntity;
	}
	,__class__: urgame_LevelItem
};
...
urgame_QuestionArea.prototype = {
	...
	,onSuccess: function(pack) {
		...
		this.getQuestions();
		urgame_Signals.showLevel.emit();
	}
	,getQuestions: function() {
		this.allQuestions = JSON.parse(urgame_QuestionArea.questionPack.getFile("Op" + urgame_Game.operator + "/" + this.fileName + ".json").toString());
		this.createQuestions();
	}
	,createQuestions: function() {
		if(this.questions == null) this.questions = []; else this.questions = this.questions;
		var _g1 = 0;
	...
	,displayNextQ: function() {
		if((function($this) {
			...
			return $r;
		}(this))) return this.questions[this.questionIndex++]; else if((function($this) {
			var $r;
			var a1 = $this.fileName;
			var b1 = urgame_Game.maxLevel;
			$r = (function($this) {
				var $r;
				var aNeg1 = b1 < 0;
				var bNeg1 = a1 < 0;
				$r = aNeg1 != bNeg1?aNeg1:b1 > a1;
				return $r;
			}($this));
			return $r;
		}(this))) {
...
flambe_asset_Manifest.__meta__ = { obj : { assets : [{ JSON : [{ bytes : 577, md5 : "19bde6385a626f34fa3465cfa6a149d7", name : "Op/1.json"},{ bytes : 589, md5 : "8438c684ee1d64dbec47f8f4adbfa0bc", name : "Op/10.json"},{ bytes : 575, md5 : "416acefdd80ede9056fcca8322baf42d", name : "Op/11.json"},{ bytes : 583, md5 : "653a83322cdcac982f1de80fb75fe907", name : "Op/12.json"},{ bytes : 571, md5 : "ede77d1d86d049c49309bd0fa6876256", name : "Op/13.json"},{ bytes : 586, md5 : "dc71e58fbac1914a857de67d321dd709", name : "Op/14.json"},{ bytes : 591, md5 : "a7ed798199fa70a510ba4b874a196026", name : "Op/15.json"},{ bytes : 592, md5 : "ee6c03a80d025456d186c2e6d93319f7", name : "Op/2.json"},{ bytes : 579, md5 : "4ae5fe318d60a486ab334aa54437b75c", name : "Op/3.json"},{ bytes : 570, md5 : "790736ef5e10005a4dd6c6ccd0975697", name : "Op/4.json"},{ bytes : 575, md5 : "6d5552ffbb048b93b2aeec0a79e8ee84", name : "Op/5.json"},{ bytes : 576, md5 : "aac7e0006769fc3dd00ef282dffe1957", name : "Op/6.json"},{ bytes : 589, md5 : "4d7d08e2d6d974280971e69a51bdf20d", name : "Op/7.json"},{ bytes : 580, md5 : "d0491218a4127e4d13e3e86e84f354fb", name : "Op/8.json"},{ bytes : 575, md5 : "7f22504c43bd0e36f7e1348984a13af4", name : "Op/9.json"},{ bytes : 565, md5 : "b7423c4c5360aeff06afac6336976298", name : "Op+/1.json"},{ bytes : 671, md5 : "95904b2b3f39cac3953f27c0544cdac6", name : "Op+/10.json"},{ bytes : 691, md5 : "2b8c031b1c20e72a2af36e387881e6e8", name : "Op+/11.json"},{ bytes : 703, md5 : "9d5caf3e8950b3f690769dcbb3238e01", name : "Op+/12.json"},{ bytes : 683, md5 : "3b14f96d80d1c899ec99790ee00a5630", name : "Op+/13.json"},{ bytes : 695, md5 : "d0b921274b79fc48a0a17e99375aeb31", name : "Op+/14.json"},{ bytes : 683, md5 : "a5c11d0e182ea8797fded6eceae73ffa", name : "Op+/15.json"},{ bytes : 616, md5 : "9275edb4e2679add69ca09efaf8a041d", name : "Op+/2.json"},{ bytes : 622, md5 : "ce476815989ebcbaddd0ec73fb7cecb2", name : "Op+/3.json"},{ bytes : 610, md5 : "17d62647346b1c18eed4c4a65e569abd", name : "Op+/4.json"},{ bytes : 619, md5 : "283f28e187248e3d05c4fa2dde9ec1fc", name : "Op+/5.json"},{ bytes : 667, md5 : "7dfbd630ad3243360e7c829db677c05d", name : "Op+/6.json"},{ bytes : 671, md5 : "f748b763a104c64f8a17b6c6e5eb08d9", name : "Op+/7.json"},{ bytes : 675, md5 : "932027e745055bcd975e00d1a4409e49", name : "Op+/8.json"},{ bytes : 671, md5 : "813ed69726b0d30fe737dbe7f1b3def2", name : "Op+/9.json"},{ bytes : 566, md5 : "59a0b74b6c639696ef62f243a41cbdbc", name : "Op-/1.json"},{ bytes : 594, md5 : "e86e088ab7fc2faad5c8c4709785938d", name : "Op-/10.json"},{ bytes : 608, md5 : "d6caf77af74ef60e2571d2d3948b7b27", name : "Op-/11.json"},{ bytes : 614, md5 : "0fad8bffdf95a1072ada0acd57af5224", name : "Op-/12.json"},{ bytes : 609, md5 : "6042bf1838f2f0d16b6e9fb05ac97857", name : "Op-/13.json"},{ bytes : 606, md5 : "291f199096eacc8333c64ea82e09cf3b", name : "Op-/14.json"},{ bytes : 620, md5 : "83850cbd820e17b2fa9fea596d6ed602", name : "Op-/15.json"},{ bytes : 581, md5 : "dc73d1572530c064c4e7ff1f5860275c", name : "Op-/2.json"},{ bytes : 581, md5 : "6bec4025845c41e77b5b488fd8337843", name : "Op-/3.json"},{ bytes : 598, md5 : "b24c9f9d8528f375990d95b6b17022b8", name : "Op-/4.json"},{ bytes : 593, md5 : "0190aba9a7dde88e1dacb3da5fa0dea5", name : "Op-/5.json"},{ bytes : 583, md5 : "66b0ca6f547ea6873bc7ff1202d5480f", name : "Op-/6.json"},{ bytes : 586, md5 : "4dabbc323026841b86ab1399921c8ba9", name : "Op-/7.json"},{ bytes : 583, md5 : "931f77712e261cd801b5c64a0eafa69f", name : "Op-/8.json"},{ bytes : 588, md5 : "14c9aeb18c406d7bf2a5dede41d0a2dc", name : "Op-/9.json"},{ bytes : 590, md5 : "ced25baeff47a534514e3d41090cb05c", name : "Opx/1.json"},{ bytes : 663, md5 : "4966f4191316dab5d298d041de37c668", name : "Opx/10.json"},{ bytes : 671, md5 : "666fab45d541bb01547e86fdf2d80d00", name : "Opx/11.json"},{ bytes : 671, md5 : "7b19c774287ecc3cebe0f9b0fd76c853", name : "Opx/12.json"},{ bytes : 667, md5 : "ee6f600fb50a573d6d0cb7bb28429ecc", name : "Opx/13.json"},{ bytes : 671, md5 : "07b40f892d3aefe4db8244df3f936839", name : "Opx/14.json"},{ bytes : 675, md5 : "4b7fbeb18383caa683f153c884191f13", name : "Opx/15.json"},{ bytes : 599, md5 : "e04ee5fcf683b97afa3c923727fbce17", name : "Opx/2.json"},{ bytes : 612, md5 : "96736dfb56bafa8cf8f1ce98db509c38", name : "Opx/3.json"},{ bytes : 615, md5 : "801999ea5fa0c428088cb29c6f0c7267", name : "Opx/4.json"},{ bytes : 668, md5 : "bab12497a17ef52973155493cfc5eca2", name : "Opx/5.json"},{ bytes : 671, md5 : "4b51193d535699545f645d530b4b0f97", name : "Opx/6.json"},{ bytes : 671, md5 : "fb4c5c9ec0346b01bce212dd9e26a788", name : "Opx/7.json"},{ bytes : 675, md5 : "606a2c0e7e17a3c6eaa443c2a4a28849", name : "Opx/8.json"},{ bytes : 663, md5 : "6b5f831501f1bd993bb666493ecadd3b", name : "Opx/9.json"}], bootstrap : [{ bytes : 174759, md5 : "0d9b2688c0519c631582fed77694938c", name : "background.png"},{ bytes : 14362, md5 : "e1596bccd504f6c6a78888500ba6ffbb", name : "balloon.png"},{ bytes : 2797, md5 : "a50625a21376f53b87e52b128aa96d4c", name : "balloonparticle.png"},{ bytes : 14821, md5 : "c501b67d6ac761a60eeba5b8e80ef7c4", name : "BaloonAnim/atlas0.png"},{ bytes : 22686, md5 : "d4f2560824c38a30982d4f8d7bf2593e", name : "BaloonAnim/library.json"},{ bytes : 32, md5 : "84e2cc9cd070375d953c0bcf19696cc1", name : "BaloonAnim/md5"},{ bytes : 1, md5 : "003d62073d0b493a14427a45413fc595", name : "BaloonAnim/version"},{ bytes : 7701, md5 : "519b3c48412a582296c9076f874a1d83", name : "BaloonAnim.assets/images/Bitmap 1.png.png"},{ bytes : 744, md5 : "abe48661625426bc379b115b1b1c163f", name : "fonts/Arial.bmfc"},{ bytes : 62781, md5 : "a846e319f4a9656b1e1c364e100d9878", name : "fonts/Arial.fnt"},{ bytes : 51674, md5 : "c8c5ade18141e62cd89a7a7df421bf07", name : "fonts/Arial_0.png"},{ bytes : 7701, md5 : "519b3c48412a582296c9076f874a1d83", name : "gameover.png"},{ bytes : 6225, md5 : "c49af1eec5af7129d0dc037ecb0f8684", name : "LevelButtonDown.png"},{ bytes : 4280, md5 : "1dafd479245c82ce6a71a82e67b3a6b4", name : "LevelButtonUp.png"},{ bytes : 5085, md5 : "086dce3cac319d1f33b725967ac1a431", name : "LevelSelectBG.png"},{ bytes : 10428, md5 : "644e80c0ef3c9f60d786f503caea8b89", name : "QuestionBoard@2x.png"},{ bytes : 11621, md5 : "efe0b06b7ab09b2aba72c9f3188fe2c7", name : "ScoreBoard@2x.png"},{ bytes : 11572, md5 : "02f99231f1b810e18a11a879da857ef9", name : "sounds/slip.wav"},{ bytes : 22478, md5 : "b08f472c1455045ac1c20711a7d5ab6a", name : "sounds/sound.wav"},{ bytes : 17674, md5 : "545dcca886b73d01fad95ef4256883e5", name : "TamamaSmall@2x.png"}]}]}};
...
urgame_Game.operator = "+";
urgame_Game.maxLevel = 15;
urgame_Signals.showLevel = new flambe_util_Signal0();
...
urgame_Signals.startGame = new flambe_util_Signal1();
